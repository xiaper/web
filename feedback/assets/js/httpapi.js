...
var httpapi = {
  ...
  rateArticle: function (aid, rate) {
    ...
    /**
   * @api {get} /api/category/ticket 加载工单类别
   * @apiName initAnswer
   * @apiGroup User
   * @apiVersion 1.0
   * @apiPermission afterLogin
  ...
  getTicketCategories: function () {
    $.ajax({
      url: data.HTTP_HOST +
      "/visitor/api/category/ticket?access_token=" +
      data.passport.token.access_token,
      type: "get",
      data: {
        uid: data.adminUid,
      ...
      success:function(response){
        console.log("get ticket categories success:", response.data, response.data.length);
        if (response.status_code === 200) {
          //
          for (var i = 0; i < response.data.length; i++) {
            var category = response.data[i];
            // console.log(category.cid, category.name);
            $("#ticketCategory").append("<option value="+category.cid+">"+category.name+"</option>"); 
          }
          // 表单重新渲染，要不然添加完显示不出来新的option
          data.ticketForm.render();
        } else {
          alert(response.message);
        }
      },
      error: function(error) {
        console.log("get ticket categories error:", error);
      }
    });
  },
  /**
   * @api {post} /visitor/api/ticket/create 提交工单
   ...
   * 
   * @apiParam {String} access_token 访问令牌
   * @apiParam {String} urgent 是否紧急
   * @apiParam {String} cid 工单类别cid
   * @apiParam {String} content 内容
   * @apiParam {String} mobile 手机号
   * @apiParam {String} email 邮箱
   * @apiParam {String} fileUrl 附件url
   * @apiParam {String} client 固定写死为 'web'
   * 
   * @apiDescription 提交工单
   *
   * @apiUse ResponseResultSuccess
   */
  createTicket: function(urgent, cid, content, mobile, email, fileUrl) {
    console.log('client:' + data.client);
    $.ajax({
      url: data.HTTP_HOST +
      "/visitor/api/ticket/create?access_token=" + data.passport.token.access_token,
      contentType: "application/json; charset=utf-8",
      dataType: "json",
      type: "post",
      data: JSON.stringify({
  ...
  /**
   * @api {get} /visitor/api/category/feedback 加载意见反馈类别
   * @apiName getFeedbackCategories
   * @apiGroup Feedback
   * @apiVersion 1.0
   * @apiPermission afterLogin
   * 
  ...
  getFeedbackCategories: function () {
    $.ajax({
      url: data.HTTP_HOST +
      "/visitor/api/category/feedback?access_token=" +
      data.passport.token.access_token,
      type: "get",
      data: {
        uid: data.adminUid,
      ...
      success:function(response){
        console.log("get feedback categories success:", response.data, response.data.length);
        if (response.status_code === 200) {
          //
          for (var i = 0; i < response.data.length; i++) {
            var category = response.data[i];
            // console.log(category.cid, category.name);
            $("#feedbackCategory").append("<option value="+category.cid+">"+category.name+"</option>"); 
          }
          // 表单重新渲染，要不然添加完显示不出来新的option
          data.feedbackForm.render();
        } else {
          alert(response.message);
        }
      },
      error: function(error) {
        console.log("get feedback categories error:", error);
      }
    });
  },
  /**
   * @api {post} /visitor/api/feedback/create 提交工单
   ...
   * @apiUse ResponseResultSuccess
   */
  createFeedback: function(cid, content, mobile, email, fileUrl) {
    console.log('client:' + data.client);
    $.ajax({
      url: data.HTTP_HOST +
      "/visitor/api/feedback/create?access_token=" + data.passport.token.access_token,
      contentType: "application/json; charset=utf-8",
      dataType: "json",
      type: "post",
      data: JSON.stringify({
      ...
      success:function(response){
        console.log("create feedback success: ", response);
        if (response.status_code == 200) {
          data.feedbackLayer.msg('意见反馈成功');
          // disable 提交按钮，防止重复提交
